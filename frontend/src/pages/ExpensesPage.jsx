...
export default function ExpensesPage() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      const expData = Array.isArray(expRes.data) ? expRes.data : expRes.data.content || [];
      setExpenses(expData);
  ...
  const handleDelete = async (row) => {
    ...
    try { await deleteExpense(row.id); fetchAll(); } catch (err) { console.error(err); }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) { await updateExpense(editing.id, payload); } else { await createExpense(payload); }
      setModalOpen(false);
      fetchAll();
  ...
  return (
    <div className="page">
      ...
      <div className="page__header">
        <h2 className="page__subtitle">Expenses</h2>
      ...
      {loading ? (
        ...
        <DataTable columns={columns} data={expenses} onEdit={openEdit} onDelete={handleDelete} />
      )}
